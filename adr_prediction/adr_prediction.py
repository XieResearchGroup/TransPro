...
if __name__ == '__main__':
    ...
    cur_model_name = 'deepNN'
    # 'extratrees' 'randomforest''knn''deepNN

    # set up logging system
    logging.basicConfig(filename='/raid/home/yoyowu/PertPro/adr_prediction/logs/deepNN/'+ args.cur_dataset +'_'+ args.split_method +'_'+ cur_model_name.lower() + '_seed' +str(args.seed)+'_' + gx_file.rsplit('/',1)[1].rsplit('.',1)[0].lower(), 
                    level=logging.DEBUG,
    ...
    if args.split_method == "scaffold_split":
        ...
        train_index,dev_index,test_index = data_preparer.random_scaffold_split(X,data_preparer.smiles_all,seed=args.seed,frac_train=0.8,frac_test=0.2)
    ...
    cur_model = DeepModel(X_dim = X.shape[1], Y_dim = sum(nozero_filter), layout = deep_model_layout,random_seed=args.seed, device = device, split_num=split_num, logger = logger)
    ...
    if isinstance(cur_model, DeepModel):
        ...
        # Y_pred = np.transpose([pred[:, 1] for pred in Y_pred])
        aucroc_macro = roc_auc_score(Y_truth.reshape(-1), Y_pred.reshape(-1))
        prauc_macro = average_precision_score(Y_truth.reshape(-1), Y_pred.reshape(-1))
        aucroc_micro = roc_auc_score(Y_truth, Y_pred, average = 'micro')
        prauc_micro = average_precision_score(Y_truth, Y_pred, average = 'micro')
